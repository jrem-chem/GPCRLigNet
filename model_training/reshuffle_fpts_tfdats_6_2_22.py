...
def gen_roc(predicted,truth,cutts):
    ...
    np.save("cicular_4_models_6_17_21/roc_dat_"+str(fpt_type),[false_poss,true_poss])
...
train_dataset=tf.data.experimental.load("data_sets/new_data_6_13_21/train_dataset_"+fpt_type+"_"+str(tmod),element_spec =(tf.TensorSpec(shape=( fpt_sizes[fpt_type]), dtype=tf.float32, name=None), tf.TensorSpec(shape=( 2), dtype=tf.float32, name=None)))
test_dataset=tf.data.experimental.load("data_sets/new_data_6_13_21/test_dataset_"+fpt_type+"_"+str(tmod),element_spec =(tf.TensorSpec(shape=( fpt_sizes[fpt_type]), dtype=tf.float32, name=None), tf.TensorSpec(shape=( 2), dtype=tf.float32, name=None)))
n_dat=tf.data.experimental.cardinality(train_dataset).numpy()+tf.data.experimental.cardinality(test_dataset).numpy()
print("concatenating")
#concat train and test
full_dataset=train_dataset.concatenate(test_dataset)
#shuffle full dataset
print("shuffling")
shuffled_dataset=full_dataset.shuffle(buffer_size=n_dat)

print("splitting")
#now split again
shuffled_train_dataset=shuffled_dataset.take(int((1-v_split)*n_dat))
shuffled_test_dataset=shuffled_dataset.take(int((v_split)*n_dat))

print("saving")
#save new datasets
tf.data.experimental.save(shuffled_train_dataset, "data_sets/new_data_6_13_21/train_dataset_"+fpt_type+"_"+str(tmod)+"_"+str(shuf_i))
tf.data.experimental.save(shuffled_test_dataset, "data_sets/new_data_6_13_21/test_dataset_"+fpt_type+"_"+str(tmod)+"_"+str(shuf_i))
...
if retrain==1:
    ...
    with mirrored_strategy.scope():
        ...
        for i in range(n_dense):
            ...
            if i ==0:
                ...
                dense_outs.append(dense_layers[-1](input_fpts))
            else:
        ...
        log_dir = "cicular_4_models_6_17_21/t_board_"+str(fpt_type)
    ...
    model.save("cicular_4_models_6_17_21/model_"+str(fpt_type)+".tf")
    np.save("cicular_4_models_6_17_21/model_"+str(fpt_type)+"_config",model_config)
    #exit()
else:
    #mirrored_strategy = tf.distribute.MirroredStrategy()
    #with mirrored_strategy.scope():
    model=keras.models.load_model("cicular_4_models_6_17_21/model_"+str(fpt_type)+".tf")
    model.compile(
            loss=crossentropy,#keras.losses.MeanSquaredError(),
            optimizer=keras.optimizers.Adam(learning_rate=learn_r),
            metrics=["MeanSquaredError"],
