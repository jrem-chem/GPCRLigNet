...
def gen_roc(predicted,truth,cutts):
    ...
    np.save("roc_compare/roc_dat_"+str(fpt_type)+"_"+str(model_i),[false_poss,true_poss])
...
train_dataset=tf.data.experimental.load(train_dataset_path,element_spec =(tf.TensorSpec(shape=( 1024), dtype=tf.float32, name=None), tf.TensorSpec(shape=( 2), dtype=tf.float32, name=None)))
test_dataset=tf.data.experimental.load(test_dataset_path,element_spec =(tf.TensorSpec(shape=( 1024), dtype=tf.float32, name=None), tf.TensorSpec(shape=( 2), dtype=tf.float32, name=None)))
...
if retrain==1:
    ...
    with mirrored_strategy.scope():
        ...
        log_dir = "roc_compare_6_13_21/t_board_"+str(fpt_type)
    ...
    model.save("roc_compare_6_13_21/model_"+str(fpt_type)+"_"+str(model_i)+".tf")
    np.save("roc_compare_6_13_21/model_"+str(fpt_type)+"_config_"+str(model_i)+"",model_config)
    #exit()
else:
    #mirrored_strategy = tf.distribute.MirroredStrategy()
    #with mirrored_strategy.scope():
    model=keras.models.load_model("roc_compare_6_13_21/model_"+str(fpt_type)+"_"+str(model_i)+".tf",custom_objects={"crossentropy":crossentropy})
    model.compile(
            loss=crossentropy,#keras.losses.MeanSquaredError(),
            optimizer=keras.optimizers.Adam(learning_rate=learn_r),
            metrics=["MeanSquaredError"],
